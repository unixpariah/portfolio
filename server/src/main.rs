...
static SECRET: LazyLock<String> = LazyLock::new(|| {
    let secret = std::fs::read_to_string("/run/secrets/github_api").unwrap_or_default();
                    ...
                    if let Ok(json) = response.json::<Value>().await {
                        if let Some(repos) = json.as_array() {
                            let futures = repos.iter().filter_map(|repo| {
                                ...
                                if let (Some(id), Some(name), Some(description), Some(stars)) = (
                                    ...
                                    Some(async move {
                                        ...
                                        let languages = match json.take() {
                                            Value::Object(map) => map
                                                .into_iter()
                                                .map(|(k, v)| (k, v.as_i64().unwrap_or_default()))
